  ...
  const setYealryPlan = () => {
    ...
    return ( isYearly &&
      <Container name='yearInput'>
        ...
        <ObjectInputBox name='yearPlan' content="1년 동안 이루고 싶은 목표를 적어주세요!" value={yearlyPlanInput.current?.yearPlan}/>

        <ObjectInputTitle numbering={2} title="반년 목표" />
        <ObjectInputBox name='halfPlan' content="반년 동안 이루고 싶은 목표를 적어주세요!"/>
        ...
        { [1, 2, 3, 4, 5, 6].map((e) => (
          <MonthlyObjectInput name={'monthlyPlan' + e} month={e} key={'month' + e} />
        ))}
  
        <Caution>* 반 년이 지난 후 나머지 목표는 반 년 이후에 쓸 거예요!</Caution>
        <ObjectInputTitle numbering={4} title="일 별 목표" />
        { ['월', '화', '수', '목', '금', '토', '일'].map((e) => (
          <DailyInput day={e} name={'dailyPlan' + e} key={'day' + e} />
        ))}
      </Container>
    )
  }
